(() => {
  ...
  const selectNomadan = document.querySelector('.nomadan');
  ...
  def.textContent = `${nomadan}`;
  //設定した最終的な経験値
  let nomadanSum = 2200;
  const final = document.querySelector('.final');
  final.textContent = `${nomadanSum}`;
  ...
  //ベル使用
  selectNomadan.addEventListener('change', (e) => SelectNomadan(e));
